...
class ArchivesConverter(Converter):
    ...
    def process(self, source: Path, requested: Path, dest: Path) -> None:
        ...
        if top == 'formats':
            ...
            dest.write_bytes(_write_archive(members, fmt, level=None))
            return
        ...
        if top == 'compression':
            ...
            if fmt == 'zip':
                dest.write_bytes(
                    _write_archive(members, 'zip', level=self._ZIP_LEVELS[level_name])
                )
                return
            dest.write_bytes(
                _write_archive(members, '7z', level=self._SEVENZ_LEVELS[level_name])
            )
            return
