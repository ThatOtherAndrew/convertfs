...
def test_subtitles_converter_emits_each_format(
	tmp_path: Path, out_ext: str, expected_marker: str, convert_bytes,
) -> None:
	source = tmp_path / 'in.srt'
	_write_srt(source)

	output = convert_bytes(
		SubtitlesConverter(), source, tmp_path / f'out.{out_ext}'
	)

	assert expected_marker in output.decode('utf-8', errors='replace')


def test_subtitles_microdvd_includes_fps_marker(tmp_path: Path, convert_bytes) -> None:
	...
	output = convert_bytes(SubtitlesConverter(), source, tmp_path / 'out.sub')
...
def test_subtitles_round_trip_preserves_event_count(tmp_path: Path, convert_bytes) -> None:
	source = tmp_path / 'in.srt'
	_write_srt(source)

	output = convert_bytes(SubtitlesConverter(), source, tmp_path / 'out.vtt')
...
def test_subtitles_rejects_unknown_output_format(tmp_path: Path) -> None:
	...
	with pytest.raises(ValueError, match='Unsupported subtitle output format'):
		SubtitlesConverter().process(source, tmp_path / 'out.bogus', tmp_path / 'dest.bin')


def test_subtitles_accepts_vtt_input(tmp_path: Path, convert_bytes) -> None:
	...
	output = convert_bytes(SubtitlesConverter(), source, tmp_path / 'out.srt')
