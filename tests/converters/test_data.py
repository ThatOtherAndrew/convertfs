...
def test_data_json_to_yaml_round_trips_payload(tmp_path: Path) -> None:
	source = _write(
		tmp_path / 'cfg.json',
		json.dumps({'name': 'alice', 'tags': ['x', 'y'], 'count': 3}),
	)

	output = DataConverter().process(source, tmp_path / 'cfg.yaml')

	parsed = yaml.safe_load(output.decode('utf-8'))
	assert parsed == {'name': 'alice', 'tags': ['x', 'y'], 'count': 3}


def test_data_yaml_to_json_preserves_unicode(tmp_path: Path) -> None:
	source = _write(tmp_path / 'cfg.yaml', 'greeting: "Hello — 世界"\n')

	output = DataConverter().process(source, tmp_path / 'cfg.json')

	parsed = json.loads(output.decode('utf-8'))
	assert parsed == {'greeting': 'Hello — 世界'}


def test_data_toml_to_yaml_handles_nested_tables(tmp_path: Path) -> None:
	source = _write(
		tmp_path / 'cfg.toml',
		'[server]\nhost = "localhost"\nport = 8080\n\n[server.tls]\nenabled = true\n',
	)
...
def test_data_yaml_to_toml_outputs_valid_toml(tmp_path: Path) -> None:
	source = _write(tmp_path / 'cfg.yaml', 'a: 1\nb: hello\nc:\n  d: 2\n')

	output = DataConverter().process(source, tmp_path / 'cfg.toml')
...
def test_data_toml_rejects_non_mapping_root(tmp_path: Path) -> None:
	...
	with pytest.raises(ValueError, match='top-level mapping'):
		DataConverter().process(source, tmp_path / 'cfg.toml')


def test_data_xml_to_json_uses_repeated_elements_as_lists(tmp_path: Path) -> None:
	source = _write(
		tmp_path / 'cfg.xml',
		'<?xml version="1.0"?>\n'
...
def test_data_xml_to_json_uses_item_tag_as_list(tmp_path: Path) -> None:
...
def test_data_json_to_xml_emits_declaration_and_safe_root_name(
	...
	output = DataConverter().process(source, tmp_path / '99-bottles.xml')
...
def test_data_json_to_xml_handles_lists_as_item_elements(tmp_path: Path) -> None:
	source = _write(tmp_path / 'cfg.json', json.dumps({'colors': ['red', 'blue']}))

	output = DataConverter().process(source, tmp_path / 'cfg.xml')
...
def test_data_yml_alias_resolves_to_yaml_loader(tmp_path: Path) -> None:
...
def test_data_rejects_unsupported_extension(tmp_path: Path) -> None:
	...
	with pytest.raises(ValueError, match='Unsupported data conversion'):
		DataConverter().process(source, tmp_path / 'cfg.ini')
