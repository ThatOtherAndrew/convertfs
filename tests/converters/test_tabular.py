...
def test_tabular_csv_to_tsv_swaps_delimiter(tmp_path: Path) -> None:
	source = _write_csv(tmp_path / 'data.csv', [['a', 'b'], ['1', '2']])

	output = TabularConverter().process(source, tmp_path / 'data.tsv')

	text = output.decode('utf-8')
	assert text == 'a\tb\n1\t2\n'


def test_tabular_tsv_to_csv_swaps_delimiter(tmp_path: Path) -> None:
	source = tmp_path / 'data.tsv'
	source.write_text('a\tb\n1\t2\n', encoding='utf-8')

	output = TabularConverter().process(source, tmp_path / 'data.csv')
...
def test_tabular_csv_to_xlsx_round_trips_via_openpyxl(tmp_path: Path) -> None:
	rows = [['header1', 'header2'], ['x', 'y'], ['1', '2']]
	source = _write_csv(tmp_path / 'data.csv', rows)

	output = TabularConverter().process(source, tmp_path / 'data.xlsx')
...
def test_tabular_xlsx_to_csv_extracts_active_sheet(tmp_path: Path) -> None:
...
def test_tabular_csv_to_csv_is_no_op_passthrough(tmp_path: Path) -> None:
...
def test_tabular_xlsx_to_xlsx_is_no_op_passthrough(tmp_path: Path) -> None:
...
def test_tabular_csv_handles_quoted_fields(tmp_path: Path) -> None:
	source = tmp_path / 'data.csv'
...
def test_tabular_handles_empty_input(tmp_path: Path) -> None:
	source = tmp_path / 'empty.csv'
	source.write_text('', encoding='utf-8')

	output = TabularConverter().process(source, tmp_path / 'empty.tsv')
...
def test_tabular_xlsx_to_xlsx_passthrough_preserves_multiple_sheets(
	...
	output = TabularConverter().process(source, tmp_path / 'multi.xlsx')
...
def test_tabular_rejects_unsupported_output(tmp_path: Path) -> None:
	...
	with pytest.raises(ValueError, match='Unsupported tabular output'):
		TabularConverter().process(source, tmp_path / 'data.bogus')
