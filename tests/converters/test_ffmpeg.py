...
def test_ffmpeg_converter_same_container_is_passthrough(tmp_path: Path) -> None:
	...
	# semantically; the converter short-circuits and copies the source
	# verbatim, avoiding a useless encode/mux pass.
	source = tmp_path / 'clip.mp4'
	_make_noise_mp4(source)
	original = source.read_bytes()

	dest = tmp_path / 'clip.converted.mp4'
	FFMpegConverter().process(source, tmp_path / 'clip.converted.mp4', dest)
...
def test_ffmpeg_converter_remux_preserves_video_stream_codec(tmp_path: Path) -> None:
	...
	dest = tmp_path / 'clip.converted.mkv'
	FFMpegConverter().process(source, tmp_path / 'clip.converted.mkv', dest)
...
def test_ffmpeg_converter_avi_to_mkv_round_trip(tmp_path: Path) -> None:
	...
	with av.open(str(dest), mode='r') as outc:
