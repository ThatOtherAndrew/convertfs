...
def test_fonts_converter_ttf_to_woff_wraps_in_zlib(sample_ttf: Path) -> None:
	output = FontsConverter().process(
		sample_ttf, sample_ttf.with_name('out.woff'),
	)
...
def test_fonts_converter_ttf_to_woff2_uses_brotli_wrapper(sample_ttf: Path) -> None:
	output = FontsConverter().process(
		sample_ttf, sample_ttf.with_name('out.woff2'),
	)
...
def test_fonts_converter_woff_round_trip_back_to_ttf(sample_ttf: Path) -> None:
	woff = FontsConverter().process(
		sample_ttf, sample_ttf.with_name('inter.woff'),
	)
	woff_path = sample_ttf.with_name('inter.woff')
	woff_path.write_bytes(woff)

	back_path = sample_ttf.with_name('back.ttf')
	out = FontsConverter().process(woff_path, back_path)
...
def test_fonts_converter_no_op_ttf_to_ttf_passthrough(sample_ttf: Path) -> None:
	# Source bytes should round-trip verbatim — no fontTools repack.
	original = sample_ttf.read_bytes()
	output = FontsConverter().process(sample_ttf, sample_ttf.with_name('again.ttf'))
	assert output == original


def test_fonts_converter_ttf_otf_swap_is_passthrough(sample_ttf: Path) -> None:
	...
	original = sample_ttf.read_bytes()
	output = FontsConverter().process(sample_ttf, sample_ttf.with_name('out.otf'))
	assert output == original


def test_fonts_converter_rejects_unknown_output(sample_ttf: Path) -> None:
	with pytest.raises(ValueError, match='Unsupported font output'):
		FontsConverter().process(sample_ttf, sample_ttf.with_name('out.eot'))


def test_fonts_converter_woff_passthrough(sample_ttf: Path) -> None:
	# Same-flavour conversion (woff -> woff) should also be a no-op.
	woff = FontsConverter().process(sample_ttf, sample_ttf.with_name('w.woff'))
	woff_path = sample_ttf.with_name('w.woff')
	woff_path.write_bytes(woff)

	output = FontsConverter().process(woff_path, sample_ttf.with_name('w2.woff'))
	assert output == woff
