...
def test_audio_formats_route_emits_expected_codec(
	...
	output = convert_bytes(AudioConverter(), wav_source, Path('formats') / f'tone.{out_ext}'
	)
...
def test_audio_formats_same_extension_is_passthrough(wav_source: Path, convert_bytes) -> None:
	original = wav_source.read_bytes()

	output = convert_bytes(AudioConverter(), wav_source, Path('formats') / 'tone.wav')

	assert output == original


def test_audio_quality_levels_apply_distinct_bitrates(wav_source: Path, convert_bytes) -> None:
	# Pick mp3 because its bitrate dial spans a clear range from very-low
	# (64k) to very-high (320k).
	low = convert_bytes(AudioConverter(), wav_source, Path('quality') / 'tone.very-low.mp3'
	)
	high = convert_bytes(AudioConverter(), wav_source, Path('quality') / 'tone.very-high.mp3'
	)
...
def test_audio_quality_rejects_unknown_level(tmp_path: Path, wav_source: Path) -> None:
...
def test_audio_bitrate_route_applies_explicit_kbps(
	...
	output = convert_bytes(AudioConverter(), wav_source, Path('bitrate') / f'tone.{rate}k.mp3'
	)
...
def test_audio_bitrate_rejects_non_mp3_extension(tmp_path: Path, wav_source: Path) -> None:
...
def test_audio_preset_podcast_outputs_mono_mp3(wav_source: Path, convert_bytes) -> None:
	output = convert_bytes(AudioConverter(), wav_source, Path('presets') / 'tone.podcast.mp3'
	)
...
def test_audio_preset_audiobook_outputs_aac_mono_22050(wav_source: Path, convert_bytes) -> None:
	output = convert_bytes(AudioConverter(), wav_source, Path('presets') / 'tone.audiobook.m4a'
	)
...
def test_audio_preset_music_cd_outputs_stereo_flac(stereo_wav_source: Path, convert_bytes) -> None:
	output = convert_bytes(AudioConverter(), stereo_wav_source, Path('presets') / 'tone.music-cd.flac'
	)
...
def test_audio_preset_music_hires_uses_96khz_rate(stereo_wav_source: Path, convert_bytes) -> None:
	output = convert_bytes(AudioConverter(), stereo_wav_source, Path('presets') / 'tone.music-hires.flac'
	)
...
def test_audio_preset_voice_memo_outputs_mono_opus(wav_source: Path, convert_bytes) -> None:
	output = convert_bytes(AudioConverter(), wav_source, Path('presets') / 'tone.voice-memo.opus'
	)
...
def test_audio_preset_rejects_mismatched_extension(tmp_path: Path, wav_source: Path) -> None:
...
def test_audio_unsupported_top_level_path(tmp_path: Path, wav_source: Path) -> None:
