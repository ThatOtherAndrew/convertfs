...
def test_archives_zip_to_each_format_round_trips_members(
	...
	output = convert_bytes(ArchivesConverter(), source, Path('formats') /f'src.{out_suffix}'
	)

	assert reader(output, **reader_kwargs) == dict(_MEMBERS)


def test_archives_tar_xz_input_round_trips_to_zip(tmp_path: Path, convert_bytes) -> None:
	source = _build_tar(tmp_path / 'src.tar.xz', _MEMBERS, compression='xz')

	output = convert_bytes(ArchivesConverter(), source, Path('formats') /'src.zip'
	)

	assert _read_zip(output) == dict(_MEMBERS)


def test_archives_7z_input_round_trips_to_tar_gz(tmp_path: Path, convert_bytes) -> None:
	source = _build_7z(tmp_path / 'src.7z', _MEMBERS)

	output = convert_bytes(ArchivesConverter(), source, Path('formats') /'src.tar.gz'
	)

	assert _read_tar(output, compression='gz') == dict(_MEMBERS)


def test_archives_zip_to_zip_format_subdir_is_passthrough(tmp_path: Path, convert_bytes) -> None:
...
def test_archives_tar_gz_to_tgz_alias_is_passthrough(tmp_path: Path, convert_bytes) -> None:
	...
	output = convert_bytes(ArchivesConverter(), source, Path('formats') /'src.tgz'
	)
...
def test_archives_compression_levels_produce_smaller_files_at_max(
	...
	fast = convert_bytes(ArchivesConverter(), source, Path('compression') /'src.fast.zip'
	)
	balanced = convert_bytes(ArchivesConverter(), source, Path('compression') /'src.balanced.zip'
	)
	max_ = convert_bytes(ArchivesConverter(), source, Path('compression') /'src.max.zip'
	)
...
def test_archives_7z_compression_levels_extract_correctly(tmp_path: Path, convert_bytes) -> None:
	source = _build_zip(tmp_path / 'src.zip', _MEMBERS)

	output = convert_bytes(ArchivesConverter(), source, Path('compression') /'src.balanced.7z'
	)
...
def test_archives_emits_format_specific_magic_bytes(tmp_path: Path, convert_bytes) -> None:
	...
	source = _build_zip(tmp_path / 'src.zip', _MEMBERS)
	conv = ArchivesConverter()

	zip_out = convert_bytes(conv, source, Path('formats') / 'src.zip')
	tar_out = convert_bytes(conv, source, Path('formats') / 'src.tar')
	tgz_out = convert_bytes(conv, source, Path('formats') / 'src.tar.gz')
	tbz_out = convert_bytes(conv, source, Path('formats') / 'src.tar.bz2')
	txz_out = convert_bytes(conv, source, Path('formats') / 'src.tar.xz')
	sevenz_out = convert_bytes(conv, source, Path('formats') / 'src.7z')
